...
모델명 입력
  ...
  ▼ Step 2  멀티소스 교차검증 (삼성닷컴/네이버) (~10초)
  ▼ Step 3  100점 채점 (Min-Max 가중 점수)   (<1초)
  ▼ Step 4  경쟁사 탐색 및 동일 세그먼트 매칭 (~60초)
            (동일 출시년도 & 주요 스펙 일치 필수)
  ▼ Step 5  CPI/VFM 분석 및 7단계 전략 판정  (<2초)
  ▼ Step 6  경쟁사 공식몰 교차검증           (~30초)
  ▼ Step 7  최종 리포트 (전략 Verdict 제공)
```
...
| 단계 | 상태 | 비고 |
|------|------|------|
| Step 1 다나와 크롤링 | ✅ 정상 | 연도 추출 로직 강화 |
| Step 2 멀티소스 검증 | ✅ 정상 | 삼성닷컴 -> 네이버 Waterfall 패턴 |
| Step 3 100점 채점 | ✅ 정상 | 가중치 반영 점수 산출 |
| Step 4 세그먼트 매칭 | ✅ 정상 | 동일 연도/주요스펙 엄격 필터링 |
| Step 5 CPI/VFM 분석 | ✅ 정상 | 7단계 전략 Verdict 도출 |
| Step 6 경쟁사 검증 | ✅ 정상 | 네이버 스토어 Fallback 지원 |
| CSV 배치 처리 | ✅ 정상 | CPI/Verdict 포함 결과 생성 |
| pytest | ✅ 117 passed | 최신 로직 반영 확인 중 |
